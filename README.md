...
- 脚本默认根据是否预热自动选择 `repo sync -c` 项目（默认仅 `build`）；`audio_framework` 代码仍可由 `AUDIO_FRAMEWORK_DIR` 提供。
...
- `SYNC_PROJECTS`：`repo sync -c` 的项目列表。未显式设置时默认仅 `build`。
...
2. `repo sync` 同步所需项目（默认根据是否预热自动选择：默认仅 `build`；也可通过 `SYNC_PROJECTS` 覆盖）。
