...
4. `build_command`：独立构建命令（默认 `bash build/prebuilts_config.sh && hb build audio_framework -i`，可按你的环境替换）
...
这样会从 `base_ref` 拉代码，再 `cherry-pick` 指定 `pr_commit`，并拉取 `build` 仓、安装 `hb` 工具，最后执行独立构建命令。
